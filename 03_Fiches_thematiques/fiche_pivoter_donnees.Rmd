...
Cette fiche illustre les différentes méthodes de réorganisation des données avec les données de la Base Permanente des Equipements, disponibles dans le package `doremifasol`:
...
On commence par transformer la table `bpe_ens_2018` en `tibble`. Vous pouvez consulter la fiche [Manipuler des données avec le `tidyverse`] pour en apprendre davantage sur les objets `tibble`.
...
bpe_ens_2018 %>%
	...
	pivot_wider(id_cols =TYPEQU,  names_from = REG, values_from = NB_EQUIP_TOT, names_prefix = "nb_equip_reg")
```


::: {.conseil data-latex=""}

Il est tentant de restructurer les données sous format *wide* car ce format peut paraître plus intuitif. Toutefois, il est généralement plus simple et plus rigoureux de traiter les données en format *long*. En effet, l'utilisation de `pivot_wider()` aboutit fréquemment à des tables mal ordonnées, dont les noms de colonnes contiennent des informations importantes. Il est donc conseillé de bien réfléchir avant de restructurer en format *wide*, et de ne le faire que lorsque cela paraît indispensable.
...
```{r, eval=TRUE}
VADeathTIB <- as_tibble(VADeaths, rownames = "rn")
