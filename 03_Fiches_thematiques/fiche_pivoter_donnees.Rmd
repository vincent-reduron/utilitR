...
![Long to wide](./pics/datatable/longtowide.png) ![Wide to long](./pics/datatable/widetolong.png)
...
bpe2018_DT <- as.data.table(bpe_ens_2018)
...
```{r, eval = TRUE}
data.table::dcast(data = bpe2018_DT,
									formula = TYPEQU ~ REG,
									value.var = "NB_EQUIP")
```
...
head(table(bpe2018_DT$TYPEQU, bpe2018_DT$REG))
```

Ce comportement est dû au fait qu'**il n'y a pas qu'une seule valeur possible par croisement** lignes/colonnes. 

En revanche, si nous ajoutons le paramètre `fun.aggregate`, le résultat est totalement différent. Nous souhaitons par exemple calculer la somme des valeurs pour chaque "cellule" de la table.
...
## Pivoter des données avec la `tidyverse`.
