...
::: {.conseil data-latex=""}
Il est possible de réaliser des pivots avec le `tidyverse` ou `data.table`. Il est néanmoins conseillé d'être cohérent avec les outils de manipulation de données que vous utilisez. Si vous manipulez vos données avec `data.table`, il sera préférable d'utiliser les fonctions du package (`melt` et `dcast`) de ce _package_ qui est optimisée pour les objets `data.table`. Si vous manipulez vos données avec le `tidyverse`, il est recommandé d'utiliser les fonctions du *package* `tidyr` (`pivot_longer` et `pivot_wider`).

Il n'est pas recommandé d'utiliser les fonctions du package `reshape2` (`melt` et `dcast`) qui n'est plus maintenu. Les exemples disponibles sur internet utilisant ce package sont directement transposables en `data.table` car la syntaxe des fonctions de ce package reprend directement celle des fonctions de `reshape2`. 

De même, il n'est pas recommandé d'utiliser les fonctions `spread` et `gather` du package `tidyr` qui ont été remplacées par les fonctions `pivot_longer` et `pivot_wider` présentées dans cette fiche.
:::
...
Cette fiche illustre les différentes méthodes de réorganisation des données avec les données communales Filosofi 2015 et 2016 disponibles dans le package `doremifasol`. Nous utiliserons deux types de format

* Données au format *wide*:
...
```{r}
rbind(
  doremifasol::telechargerDonnees("COG_COMMUNE", date = 2018),
  doremifasol::telechargerDonnees("COG_COMMUNE", date = 2019)
)
```
