...
```{r, eval=TRUE}
library(doremifasol)
bpe_ens_2018 <- data("bpe_ens_2018")
filosofi_epci_2016 <- data("filosofi_epci_2016")[, ]
```
...
On commence par transformer les tables `bpe_ens_2018` et `filosofi_epci_2016` en `tibble`. Vous pouvez consulter la fiche [Manipuler des données avec le `tidyverse`] pour en apprendre davantage sur les objets `tibble`.
