...
* Il est recommandé d'utiliser les fonctions `pivot_longer` et `pivot_wider` du *package* `tidyr`, ou les fonctions `melt` et `dcast` du *package* `data.table` (en fontion des outils de manipulation de données utilisés);
* Il est déconseillé d'utiliser le *package* `reshape2` qui n'est plus mis à jour;
* Il est déconseillé d'utiliser les fonctions `spread` et `gather` du *package* `tidyr` qui ont été remplacées par les fonctions `pivot_longer` et `pivot_wider`.
:::
...
::: {.conseil data-latex=""}
Si vous êtes amenés à utiliser `ggplot` dans votre programme**** conversion vers format long
:::


Cette fiche illustre les différentes méthodes de réorganisation des données à partir de deux tables de données disponibles dans le _package_ `doremifasol`: la base permanente des équipements 2018 (`bpe_ens_2018`), et les données du répertoire Filosofi 2016 agrégées au niveau des EPCI (table `filosofi_epci_2016`), et disponibles dans le _package_ `doremifasol`.
...
Il est possible de réaliser des pivots avec le `tidyverse` et avec `data.table` (avec le même résultat). **Il est néanmoins conseillé d'être cohérent avec les outils de manipulation de données que vous utilisez.** Si vous manipulez vos données avec `data.table`, il sera préférable d'utiliser les fonctions `melt` et `dcast` de ce _package_ qui sont optimisées pour les objets `data.table`. Si vous manipulez vos données avec le `tidyverse`, il est recommandé d'utiliser les fonctions du *package* `tidyr` (`pivot_longer` et `pivot_wider`).
:::

## Restructurer des données avec le `tidyverse`

Cette section illustre les fonctions de restructuration de données avec les fonctions `pivot_longer` et `pivot_wider` du *package* `tidyr`. On commence par charger le _package_ `tidyr`.

```{r, echo = TRUE, message = FALSE, warning = FALSE}
library(tidyr)
```

On transforme les tables `bpe_ens_2018` et `filosofi_epci_2016` en `tibble`. Afin d'alléger la présentation, on conserve uniquement les variables sur le taux de pauvreté dans la table `filosofi_epci_2016`, et uniquement les données sur les boulangeries, les boucheries-charcuteries et les pharmacies ("B203", "B204" et "D301") dans la table `bpe_ens_2018`. Vous pouvez consulter la fiche [Manipuler des données avec le `tidyverse`] pour en apprendre davantage sur les objets `tibble` et sur la sélection de variables avec `select`.
